...
\begin{keypointbox}
\textbf{GitHub Repository: \url{https://github.com/JosipHarambasic/Blockchain-Seminar}}
\end{keypointbox}
...
\begin{itemize}
  \item \texttt{createPost(bytes32)} -- stores a new post, appends the post ID to the author's\linebreak \texttt{\_authorPostIds} mapping, and emits \texttt{PostCreated}.
  \item \texttt{createComment(uint256 postId, uint256 parentCommentId, bytes32)} -- stores a comment or reply. If \texttt{parentCommentId} is non-zero it is validated to belong to the same post. \texttt{post.commentCount} is incremented only for top-level comments (\texttt{parentCommentId == 0}); replies do not affect the counter.
...
\begin{itemize}
  \item \texttt{getPosts(uint256 offset, uint256 limit)} – returns a paginated slice of posts, newest-first, together with the total count. The limit is capped at 100 to prevent gas abuse.  \item \texttt{getPostsByAuthor(address)} -- returns all posts created by a given address, newest-first, by reading the \texttt{\_authorPostIds} mapping. This is an O(1) mapping lookup and requires no event scanning.  \item \texttt{getPost(uint256)} – single post by ID.
...
\begin{itemize}
  ...
  \item \texttt{getPostsByAuthor(address)} – calls the on-chain \texttt{getPostsByAuthor} view function to retrieve all posts for a given address without pagination; used by the My~Posts view.
...
\begin{itemize}
  \item \textbf{HomePage} -- responsive feed with a persistent sidebar for navigation. On desktop, posts are shown in a two-column card grid; on mobile the sidebar collapses and is accessible via a hamburger menu that slides it in as a drawer with a backdrop overlay. Each card displays the post title, a body preview, a deterministic avatar icon derived from the author's address, comment count, and a like button. A ``What's on your mind?'' compose bar at the top navigates to post creation. A \textbf{My~Posts} view filters the feed to the connected wallet's posts by calling \texttt{getPostsByAuthor} on-chain, with an independent loading state so it never races with the main feed.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lll}
    ...
    \hline
    \multicolumn{3}{l}{\textit{createPost}} \\
    \hline
    Create post with valid bytes32 hash & Post stored, \texttt{PostCreated} event emitted & \checkmark \\
    Stored post has correct fields & \texttt{id}, \texttt{author}, \texttt{contentHash} match & \checkmark \\
    Create post with zero hash & Revert: ``contentHash required'' & \checkmark \\
    Sequential IDs across multiple posts & Post IDs assigned in order & \checkmark \\
    \hline
    \multicolumn{3}{l}{\textit{createComment}} \\
    \hline
    Create top-level comment & Stored, attached to post, \texttt{commentCount} incremented & \checkmark \\
    Stored comment has correct fields & \texttt{author}, \texttt{postId}, \texttt{parentCommentId} match & \checkmark \\
    \texttt{CommentCreated} event emitted & Event args match comment data & \checkmark \\
    Create comment with invalid postId & Revert: ``Post not found'' & \checkmark \\
    Create comment with zero hash & Revert: ``contentHash required'' & \checkmark \\
    Multiple comments on the same post & All comments retrievable via \texttt{getPostComments} & \checkmark \\
    Create reply comment & Stored, attached to parent comment & \checkmark \\
    Reply absent from top-level comments & \texttt{getPostComments} excludes replies & \checkmark \\
    Reply does not increment \texttt{commentCount} & Post \texttt{commentCount} unchanged after reply & \checkmark \\
    Create reply with parent on different post & Revert: ``Parent/post mismatch'' & \checkmark \\
    Create reply with non-existent parent & Revert: ``Parent comment not found'' & \checkmark \\
    \hline
    \multicolumn{3}{l}{\textit{likePost}} \\
    \hline
    Like a post & \texttt{likeCount} incremented & \checkmark \\
    \texttt{PostLiked} event emitted & Event args match post and liker & \checkmark \\
    Like a post twice (same address) & Revert: ``Already liked this post'' & \checkmark \\
    Multiple users like the same post & \texttt{likeCount} equals number of unique likers & \checkmark \\
    Like a non-existent post & Revert: ``Post not found'' & \checkmark \\
    \texttt{hasLikedPost} reflects like state & Returns \texttt{false} before, \texttt{true} after liking & \checkmark \\
    \hline
    \multicolumn{3}{l}{\textit{likeComment}} \\
    \hline
    Like a comment & \texttt{likeCount} incremented & \checkmark \\
    \texttt{CommentLiked} event emitted & Event args match comment and liker & \checkmark \\
    Like a comment twice (same address) & Revert: ``Already liked this comment'' & \checkmark \\
    \texttt{hasLikedComment} reflects like state & Returns \texttt{false} before, \texttt{true} after liking & \checkmark \\
    \hline
    \multicolumn{3}{l}{\textit{getPosts (paginated)}} \\
    \hline
    No posts exist & Returns empty array and \texttt{total}=0 & \checkmark \\
    Paginated \texttt{getPosts} & Newest-first order, correct slice & \checkmark \\
    Offset and limit pagination & Second page returns remaining posts & \checkmark \\
    Offset beyond total & Returns empty array & \checkmark \\
    \hline
    \multicolumn{3}{l}{\textit{getPostsByAuthor}} \\
    \hline
    Author with no posts & Returns empty array & \checkmark \\
    Author posts returned newest-first & Correct order and count & \checkmark \\
    Posts from other authors excluded & Only caller's posts returned & \checkmark \\
    Interleaved posts across authors & Each author sees only their own posts & \checkmark \\
    Returned posts carry correct author field & \texttt{author} and \texttt{contentHash} match & \checkmark \\
    \hline
  \end{tabular}
...
\paragraph{Test scenario 4 -- My Posts.}
Account~A switches to the My~Posts view. The frontend calls \texttt{getPostsByAuthor} with Account~A's address; only Account~A's posts are returned. Account~B's posts do not appear. After Account~A disconnects, the My~Posts view is cleared and the feed reverts to the Home view.

\paragraph{Test scenario 5 -- Content integrity.}
...
\paragraph{Test scenario 6 -- Mobile layout.}
The application is loaded on a mobile viewport ($\leq$680\,px). The sidebar is hidden; a hamburger icon is shown in the top bar. Tapping it slides the sidebar in from the left over a semi-transparent backdrop. Tapping the backdrop or selecting a navigation item closes the sidebar. Posts are displayed in a single-column layout.
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[t]{0.32\linewidth}
    \centering
    \includegraphics[width=\linewidth]{mobile-feed.png}
    \caption{Mobile feed with hamburger menu closed.}
    \label{fig:mobile-feed}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.32\linewidth}
    \centering
    \includegraphics[width=\linewidth]{mobile-sidebar.png}
    \caption{Mobile sidebar slide-in drawer.}
    \label{fig:mobile-sidebar}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.32\linewidth}
    \centering
    \includegraphics[width=\linewidth]{mobile-post.png}
    \caption{Post detail page on mobile.}
    \label{fig:mobile-post}
  \end{minipage}
\end{figure}
...
The \texttt{getPosts} function iterates over posts in memory. With a very large number of posts this could approach the block gas limit if called from a write context, though in practice it is a \texttt{view} function and is therefore not gas-limited. Per-author queries are handled efficiently via the dedicated \texttt{\_authorPostIds} mapping and the \texttt{getPostsByAuthor} view function, avoiding the need to scan event logs. A more scalable approach for the general feed would use off-chain indexing (e.g.\ The Graph protocol) to serve paginated, filterable feeds without touching on-chain storage at query time.
...
No The~Graph subgraph is currently implemented. All feed queries go directly to the contract's \texttt{getPosts} view function, which is subject to the hard cap of 100 posts per call. Per-author queries are already handled on-chain via \texttt{getPostsByAuthor}. Deploying a subgraph that indexes \texttt{PostCreated}, \texttt{CommentCreated}, \texttt{PostLiked}, and \texttt{CommentLiked} events would enable fast, flexible querying of historical data and support features such as sorting by like count or searching by keyword.
...
The implementation covers the full stack: a Solidity smart contract deployed on UZHETH PoS, a Node.js backend for IPFS pinning, and an Angular/Ionic frontend with MetaMask integration, in-browser IPFS caching via Helia, and a fully responsive layout that works on both desktop and mobile. A dedicated \texttt{getPostsByAuthor} view function with an on-chain \texttt{\_authorPostIds} mapping enables efficient per-author queries without event scanning. End-to-end tests confirm that posts and comments written by one account are immediately readable by other accounts, the like-once invariant is enforced on-chain, and the My~Posts filter correctly isolates content per wallet.
